...
public class BrowserView : MonoBehaviour 
{
    ...
    {
        Debug.Log("ControllerRayInteractor: " + ControllerRayInteractor.CollisionInfo.Value.Point);
        Vector2 positionInHD = new Vector2();
        positionInHD.x = 1920 - ((float)(ControllerRayInteractor.CollisionInfo.Value.Point.x - 0.8) * 1000 / 2 * -1);
        positionInHD.y = 1080 - ((float)(ControllerRayInteractor.CollisionInfo.Value.Point.y - 1) * 1000 / 2);
        Debug.Log("PixelCoords: " + positionInHD);

        if (_ajc != null)
        {
            CallAjc("AddLongTap", new object[] { positionInHD.x, positionInHD.y });
        }

        // This Was the original GazeViewer we switched to ControllerRayInteractor
